...
from .serializers import JobSerializer, BookmarkSerializer, JobUpdateSerializer
...
class JobViewSet(CreateListRetrieveUpdateViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_authenticated and getattr(user,'role',None) == User.RECRUITER:
            return self.request.user.posted_jobs.annotate(application_count=Count('applications')).prefetch_related('locations')
